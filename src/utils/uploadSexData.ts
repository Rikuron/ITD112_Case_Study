...
const validateSexCSVStructure = (data: CSVRow[]): { valid: boolean; error?: string } => {
  ...
  if (!data[0].hasOwnProperty('MALE')) {
    return { valid: false, error: 'Missing required column: MALE' }
  }
  if (!data[0].hasOwnProperty('FEMALE')) {
    return { valid: false, error: 'Missing required column: FEMALE' }
  }
